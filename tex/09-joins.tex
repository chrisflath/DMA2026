...
\vspace{0.5em}
\begin{alertblock}{SQLite-Hinweis}
SQLite unterstützt FULL OUTER JOIN \textbf{nicht} direkt. Workaround: LEFT JOIN + UNION + RIGHT JOIN (oder LEFT JOIN simulieren).
\end{alertblock}
\end{frame}
...
\vspace{1em}
\textbf{Beispiel-Ausgabe (SQLite):}
\begin{verbatim}
QUERY PLAN
|--SCAN Spieler
`--SEARCH Vereine USING INDEX ...
\end{verbatim}

\vspace{1em}
\textbf{Interpretation:}
\begin{itemize}
    \item \texttt{SCAN}: Alle Zeilen durchlaufen (langsam bei großen Tabellen)
    \item \texttt{SEARCH ... USING INDEX}: Index-Lookup (schnell)
\end{itemize}
\end{frame}
