...
Wie können wir Abfragen \textbf{modular} und \textbf{wiederverwendbar} gestalten?
\end{block}
\end{frame}

\begin{frame}[fragile]{Motivation: Komplexe Abfragen}
\textbf{Problem:} Abfragen werden schnell unübersichtlich
...
\begin{itemize}
    ...
    \item Fehleranfällig bei Änderungen
\end{itemize}
\end{alertblock}
\end{frame}
...
\begin{center}
\begin{tikzpicture}
    \node[roadmapbox, minimum width=3.5cm] (sub) at (0,0) {Subqueries};
    \node[roadmapbox, minimum width=3.5cm] (cte) at (5,0) {CTEs (WITH)};
    \node[roadmapbox, minimum width=3.5cm] (view) at (10,0) {Views};
    ...
    \node[font=\footnotesize, text width=3cm, align=center] at (0,-1.2) {Abfrage in\\Abfrage};
    \node[font=\footnotesize, text width=3cm, align=center] at (5,-1.2) {Benannte\\Zwischenergebnisse};
    \node[font=\footnotesize, text width=3cm, align=center] at (10,-1.2) {Gespeicherte\\Abfrage};
\end{tikzpicture}
\end{center}

\vspace{1em}

\textbf{Und danach:} Transaktionen -- wie Änderungen sicher durchgeführt werden
\end{frame}
...
\textbf{Gibt genau einen Wert zurück} -- kann überall stehen, wo ein Wert erwartet wird
...
\textbf{Gibt eine Liste von Werten zurück}
...
\begin{itemize}
    ...
    \item \texttt{ANY/SOME}: mind. einer erfüllt
    \item \texttt{ALL}: alle erfüllen
\end{itemize}
\end{column}
...
\textbf{Referenziert die äußere Abfrage} -- wird für jede Zeile neu ausgeführt
...
\begin{center}
\begin{tikzpicture}
    \node[querybox, minimum width=4cm, align=center] (outer) at (0,0) {Äußere Abfrage\\(m1)};
...
\begin{frame}[fragile]{EXISTS: Existenzprüfung}
\textbf{Prüft, ob Subquery Ergebnisse liefert}
...
\begin{itemize}
    ...
    \item IN: Prüft alle Werte
    \item EXISTS oft schneller bei großen Subqueries
\end{itemize}
\end{column}

\begin{column}{0.5\textwidth}
\begin{exampleblock}{Tipp}
\texttt{SELECT 1} in EXISTS -- der Wert ist egal, nur die Existenz zählt.
\end{exampleblock}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    \item Lesbarkeit: Komplexe Logik in benannte Blöcke aufteilen
...
\textbf{Für hierarchische Daten} (Org-Charts, Stücklisten, Graphen)
...
\textbf{Gespeicherte Abfrage}, die sich wie eine Tabelle verhält
...
\begin{itemize}
    ...
    \item Wird bei jedem Aufruf ausgeführt
    \item Immer aktuell
\end{itemize}
\end{column}
...
\begin{itemize}
    \item Sehr häufige, teure Abfragen\\
...
\textbf{Materialized Views} speichern das Ergebnis physisch -- schneller, aber muss aktualisiert werden. Nicht in allen Datenbanken verfügbar.
\end{exampleblock}
\end{frame}
...
\textbf{Problem:} Mehrere zusammengehörige Änderungen

\begin{center}
\begin{tikzpicture}
    \node[problembox, align=center] (p1) at (0,0) {Überweisung:\\100EUR von A nach B};
...
\textbf{Lösung:} Transaktionen -- ``Alles oder nichts''
\end{frame}
...
\begin{description}
    \item[Atomicity:] Transaktion wird ganz oder gar nicht ausgeführt
    \item[Consistency:] Datenbank bleibt in konsistentem Zustand
    \item[Isolation:] Parallele Transaktionen beeinflussen sich nicht
    \item[Durability:] Bestätigte Änderungen überleben Systemausfälle
\end{description}
\end{frame}
...
\textbf{COMMIT:}
\begin{itemize}
    \item Alle Änderungen werden permanent
...
\textbf{ROLLBACK:}
\begin{itemize}
    \item Alle Änderungen werden verworfen
...
\begin{frame}[fragile]{Beispiel: Sichere Überweisung}
\begin{lstlisting}
BEGIN TRANSACTION;
...
\textbf{Zwei Transaktionen überschreiben sich gegenseitig}
...
\textbf{Problem:} T2's Änderung (+20EUR) geht verloren, weil T1 den alten Wert überschreibt.
\end{frame}
...
\textbf{Problem:} T2 arbeitet mit Daten, die T1 später zurückrollt.
\end{frame}

\begin{frame}{Isolation Levels}
\textbf{Trade-off:} Mehr Isolation = weniger Parallelität
...
\hline
READ UNCOMMITTED & \xmark & \xmark & \xmark & ⚡⚡⚡ \\
\hline
READ COMMITTED & \cmark & \xmark & \xmark & ⚡⚡ \\
\hline
REPEATABLE READ & \cmark & \cmark & \xmark & ⚡ \\
\hline
SERIALIZABLE & \cmark & \cmark & \cmark & 🐢 \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{SERIALIZABLE} verhält sich wie sequentielle Ausführung
...
\begin{itemize}
    ...
    \item Korreliert: Referenz auf äußere Abfrage
    \item EXISTS: Existenzprüfung
\end{itemize}
...
\begin{itemize}
    ...
    \item CASE WHEN für Kategorisierung
